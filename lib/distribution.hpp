...
namespace clib_util::distribution
{
	...
	using formid_pair = std::pair<std::optional<UInt32>, std::optional<std::string>>;
		...
		case kFormIDMod:
			{
				...
				return std::make_pair(string::to_num<UInt32>(splitID[0], true), splitID[1]);
			}
		case kFormID:
			return std::make_pair(string::to_num<UInt32>(a_str, true), std::nullopt);
		case kMod:
